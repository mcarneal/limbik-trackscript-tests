...
describe('Check that trackscipt is recording events on vimeo players', function () {
    ...
    var capabilities = {
        ...
        'browserstack.chrome.driver' : '2.26',
    ...
    it('Should wait for video to load and then interact with the dom', mochaAsync(async function () {
        await driver.get(`${process.env.BS_LOCAL}/html5VideoTracking/html5tracking.html`)
    ...
    it('Should detect that the video type is HTML5', mochaAsync(async function () {
        const videoType = TrackScriptAnalyzer.detectVideoType(EVENTS, 'html5')
        assert.equal(videoType, 'html5', '== should be html5')
    }))
